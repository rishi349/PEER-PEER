...
Last updated: **Phase 6 — Location, complete (backend + minimal frontend).**
Phases 1-2 and 6 are done backend+frontend; Phases 3, 4, and 5 remain
backend-only, as intended for all three — see §4c, §4d, and §4e. Phase 6
(§4f) is backend+frontend by design too, per `PHASE6_PLAN.md` §2d.
Nothing from Phase 7 onward has been started.
...
§4f/§5f below (what was built, what was verified, what wasn't) was
checked directly against the actual files in this session — `tsc
--noEmit`, `vitest run`, and `vite build` were all actually executed,
not assumed, and their real output is quoted in §5f. This HANDOFF is
written to the same standard as every prior phase's own section (compare
§4e/§5e) so this session's work can be trusted without re-auditing it
line-by-line — but "the new code's own logic doesn't need re-verifying"
is not the same as "the whole system has been run end-to-end." It
hasn't — see §5f's checklist before assuming Phase 6 works against a
real browser and a live Postgres/Redis stack.
...
│   │                            NEW §4f), Session, HierarchyEvent
│   └── src/
│       ├── config/              env.ts, prisma.ts, redis.ts, socket.ts (wires up
│       │                        presence.gateway.ts + p2p.gateway.ts — location has no
│       │                        Socket.IO surface, see §4f)
│       ├── common/              errors.ts (AppError + ErrorCode union — now includes
│       │                        LOCATION_SHARING_DISABLED, NEW §4f), asyncHandler.ts
...
│       │                        (§4f) reuses getVisibilityScope directly rather than
│       │                        adding a new Action here
│       ├── modules/presence/    see §4d below
│       ├── modules/p2p/         see §4e below
│       ├── modules/location/    NEW — see §4f below
│       ├── app.ts               Express app assembly + /health, /presence, /hierarchy,
│       │                        /location (NEW §4f) — P2P still has no REST surface
│       └── index.ts             entrypoint — unchanged since Phase 4 (still creates an
│                                 http.Server + boots Socket.IO + the presence keyspace
│                                 listener; location needed no new startup step, it's
│                                 REST-only, see §4f)
└── frontend/
    └── src/
        ├── api/                  client.ts (fetch wrapper + silent refresh), auth.ts,
        │                        hierarchy.ts, location.ts (NEW §4f)
        ├── state/                authStore.ts, hierarchyStore.ts, locationStore.ts
        │                        (NEW §4f) — all zustand
        ├── hooks/                useLocationSharing.ts (NEW §4f) — the only hook so far
        ├── components/
        │   ├── AuthShell, Field, RolePill, RadarSweepBackground, ProtectedRoute   (Phase 1)
        │   ├── hierarchy/        Phase 2 UI — see §4b below
        │   └── location/         NEW §4f — LocationPanel (opt-in toggle + status only,
        │                        no spatial rendering — that's Phase 7's radar)
        ├── pages/                LoginPage, RegisterPage, DashboardPage (now also
        │                        renders LocationPanel, NEW §4f)
        └── styles/               global.css (design tokens), forms.css
```

Frontend **was touched this session (Phase 6)** — the first frontend
change since Phase 2. Phases 3, 4, and 5 remained backend/control-plane
only (by design — see §4c/§4d/§4e), so the frontend was untouched
through all three; Phase 6 is backend+frontend by design too
(`PHASE6_PLAN.md` §2d), and the new files are exactly the ones listed
above: `api/location.ts`, `state/locationStore.ts`,
`hooks/useLocationSharing.ts`, `components/location/LocationPanel.tsx`
(+`.css`), and an edit to `pages/DashboardPage.tsx` to render the new
panel. No other frontend file was touched. **Still no presence UI and no
P2P/WebRTC UI** (no online/offline dot, no Socket.IO client connection,
no `RTCPeerConnection` anywhere — `socket.io-client` still isn't a
frontend dependency) — that gap is unrelated to Phase 6 and remains
exactly as described in §4d/§4e/§7.
...
presence, P2P, and now location — follow this same shape for Phase 7's
radar module.)
...
- Module shape: `*.routes.ts` → `*.controller.ts` (thin, `asyncHandler`)
  ...
  Match what the module's actual communication pattern needs, not a
  fixed template. Location also added one more small layer neither
  `presence` nor `p2p` needed on its own: `location.geo.ts`, pure math
  with zero infra dependency, kept separate from `location.service.ts`
  specifically so it's unit-testable without mocking anything — reach
  for this shape again whenever a service's logic includes a nontrivial
  pure computation (Phase 7's radar `getRadarPosition` is a likely
  candidate on the frontend side).
- **Never trust JWT claims for authorization** — every service function
  ...
  fifth time in `location.service.ts` — see presence.service.ts's
  original justification for why this stays a small per-module copy
...
- Add schema fields with the feature that needs them, not speculatively
  ...
  same persistent/ephemeral split every phase has kept).
- **Redis now carries three independent ephemeral concerns, each with
  its own namespace and TTL** — `presence:*` (30s TTL, liveness),
  `p2p:lease:*`/`p2p:leases:byUser:*` (45s TTL, authorization), and, as
  of Phase 6, `location:*` (50s TTL, spatial position — see §4f's
  `location.repository.ts` bullet for the reasoning behind that specific
  number). Three intentionally different numbers answering three
  different questions — **if Phase 7 (radar) or Phase 8 add a fourth
  ephemeral concern, give it its own distinct namespace/TTL too**, don't
  silently reuse any of the existing three for an unrelated concern.
- **Decoupling pattern (EventEmitter → gateway), used twice, is the
  ...
  deliberately does not use it.** `presenceEvents`/`p2pEvents`/
  `hierarchyEvents` all exist because *something* needs to turn a
  service-layer domain event into a live push to connected sockets.
  Location has no push surface at all this phase (§4f — pull-only by
  design, per `PHASE6_PLAN.md` §4), so `location.service.ts` has no
  emitter and nothing subscribes to it. **If Phase 7's radar ends up
  needing a live "this person's location changed" push**, that's the
  point where location would need its own emitter and a
  `location.gateway.ts` subscriber — don't add one before there's an
  actual consumer, per master spec §54's "don't over-engineer future
  functionality," but don't be surprised if Phase 7 is exactly the
  trigger for it.
- **Shared Socket.IO connection-auth, centralized since Phase 5**: don't
  ...
  future `location.gateway.ts`/Phase 7 radar gateway add) just types its
  `io.on("connection", ...)` callback as `(socket: AuthenticatedSocket) => ...`
  and reads `socket.data.user` directly — no per-gateway auth
  registration needed or wanted. Not touched this session since Phase 6
  added no gateway.
...
6. ~~Phase 6 — Location~~ — **done this session**, see §4f. Backend +
   ...
   mocked test.
7. **Phase 7 — Radar is the actual next phase now** (master spec §47
   Phase 7): distance, bearing, radar UI, leader/peer/all filters, range,
   zoom, orientation, user selection. Phase 6 was built specifically to
   hand this phase clean data: `GET /location/distance/:targetUserId`
   already returns `{distanceMeters, bearingDegrees, updatedAt}` (or a
   typed "unavailable" reason) with authorization already enforced and
   no raw coordinates ever exposed — Phase 7 should be able to consume
   this directly rather than touching `location.service.ts`'s internals.
   Read `PHASE6_PLAN.md` §4 open question 2 before deciding whether
   Radar needs a push layer (Socket.IO, `location:*` events) on top of
   Phase 6's pull-only endpoint, or whether polling while the radar
   screen is open is good enough — Phase 6 deliberately left this
   undecided rather than guessing at Phase 7's needs prematurely (master
   spec §54). Master spec §38's component breakdown
   (`RadarContainer`/`RadarCanvas`/`RadarControls`/`RadarUserMarker`/
   etc., with `getDistance`/`getBearing`/`getRadarPosition` kept
   independent of rendering and unit-tested on their own) is the
   template to follow — note that `location.geo.ts`'s
   `haversineDistanceMeters`/`initialBearingDegrees` already exist
   server-side and compute exactly the first two of those three; Phase 7
   only needs to add the client-side `getRadarPosition` (range/
   orientation → x/y) on top of numbers the backend already provides,
   not recompute distance/bearing itself.
8. **No frontend presence UI and no frontend WebRTC client exist yet.**
   Phase 4 and Phase 5 were both scoped backend-only to match master
   spec §47's own per-phase lists (see §4d/§4e), and Phase 6's own
   frontend work (§4f) didn't touch this gap either — `LocationPanel` is
   a settings toggle, not a presence indicator. A future session could
   ...
   reacts to `p2p:session-revoked` by closing the connection (§4e). This
   is not on the critical path to Phase 7 and could be done before,
   after, or interleaved with it — use judgment, or ask the user.
9. Also still open, not blocking Phase 7 but worth doing whenever there's
   a real Postgres/Redis/two-browser environment available: the full
   end-to-end verification checklists in §5c/§5d/§5e/§5f (backend) and
   ...
   claim, nobody has ever run two real Socket.IO clients at once to
   verify P2P signaling or revocation, and nobody has ever opened a real
   browser to exercise Phase 6's geolocation loop.** That combination is
   the single biggest unknown-unknown risk in the project right now —
   worth prioritizing over new Phase 7 features if a normal
   (non-sandboxed) environment becomes available. If such an environment
   becomes available, doing §5e's checklist before §5d's before §5f's is
   the more urgent order: presence's worst-case failure mode (a stale
   online indicator) is a UX annoyance, P2P revocation's worst-case
   failure mode (a removed user's session silently not being revoked) is
   closer to a security gap, and location's own worst-case failure modes
   (§5f's checklist) sit in between — privacy-relevant but not as sharp
   an edge as an un-revoked P2P session.

Do not start Phase 8+ before Phase 7 is solid, and don't treat Phase 4's
multi-server claim, Phase 5's revocation-bridge claim, or Phase 6's
opt-in-enforcement/TTL-expiry claims as verified until §5d's, §5e's, and
§5f's checklists have actually been run once against real, live
processes.
